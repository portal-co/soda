...
pub fn gen_trampolines(b: &mut FunctionBody, m: &mut Module, t: &mut TCache,seal: bool) -> anyhow::Result<()> {
    // b.value_locals = PerEntity::default();
    ...
    loop {
        ...
        let k = match m.get(&fun) {
            ...
            None => {
                ...
                let l = crate::copying::func::clone_fn(b, &v);
...
pub fn untcore_pass(
    ...
    loop {
        ...
        let (block, fun, args) = 'gather: loop {
            ...
            // let mut d = d.clone();
            let (Terminator::ReturnCall { func: _, args } | Terminator::ReturnCallIndirect { sig: _, table: _, args }) = d.terminator.clone() else {
                continue 'gather;
            };
        ...
        drop(e);
        let c = match fun{
            Terminator::ReturnCall { func, args } => Operator::Call { function_index: func },
            Terminator::ReturnCallIndirect { sig, table, args } => Operator::CallIndirect { sig_index: sig, table_index: table },
        ...
        let c = crate::results_ref(m, b, c).context("getting results")?;
        b.blocks[block].terminator = Terminator::None;
        b.set_terminator(
            block,
            Terminator::Return { values: c },
        );
        b.recompute_edges();
    }
}
