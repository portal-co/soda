...
impl<T: Deref<Target = waffle::Module<'static>>> Deref for Flix<T> {
    type Target = waffle::FunctionBody;

    fn deref(&self) -> &Self::Target {
        return self.mo.funcs[self.fun].body().unwrap();
    }
}
impl<T: Deref<Target = waffle::Module<'static>> + DerefMut> DerefMut for Flix<T> {
...
pub fn x2i(x: ExportKind) -> ImportKind {
    match x {
        waffle::ExportKind::Table(t) => ImportKind::Table(t),
        waffle::ExportKind::Func(f) => ImportKind::Func(f),
        waffle::ExportKind::Global(g) => ImportKind::Global(g),
        waffle::ExportKind::Memory(m) => ImportKind::Memory(m),
    }
}
pub fn i2x(i: ImportKind) -> ExportKind{
    match i{
        ImportKind::Table(t) => ExportKind::Table(t),
        ImportKind::Func(f) => ExportKind::Func(f),
        ImportKind::Global(g) => ExportKind::Global(g),
        ImportKind::Memory(m) => ExportKind::Memory(m),
    }
}
...
pub fn add_start(m: &mut Module, tf: Func) {
    ...
    let f = m.funcs.push(waffle::FuncDecl::Body(s, format!("start"), f));
    m.start_func = Some(f);
}
