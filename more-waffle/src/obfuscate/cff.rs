...
use crate::copying::block::clone_block;
use crate::{locals, values};
...
pub fn flat_map<T, E, D: Deref<Target = waffle::FunctionBody> + DerefMut>(
    f: &mut D,
    b: Block,
...
pub fn render(a: BTreeMap<usize, usize>, m: &mut Module, t: Type) -> Func {
    ...
    for (d, c) in a.clone() {
        ...
        let v = f.add_value(ValueDef::Operator(
            match t {
                waffle::Type::I32 => Operator::I32Const { value: c as u32 },
                waffle::Type::I64 => Operator::I64Const { value: c as u64 },
                waffle::Type::F32 => Operator::F32Const { value: c as u32 },
                waffle::Type::F64 => Operator::F64Const { value: c as u64 },
                waffle::Type::V128 => todo!(),
                waffle::Type::FuncRef => todo!(),
            },
            ai.clone(),
            to,
        ));
    ...
    return m
        .funcs
        .push(waffle::FuncDecl::Body(s, format!("{:?}", a), f));
}
    ...
    fn fake(&mut self, f: &mut FunctionBody, b: Block, t: Type) -> Value {
        loop {
            if self.0.gen_bool(0.1) {
                ...
                let v = f.add_value(ValueDef::Operator(
                    match t {
                        waffle::Type::I32 => Operator::I32Const {
                            value: self.0.gen(),
                        },
                        waffle::Type::I64 => Operator::I64Const {
                            value: self.0.gen(),
                        },
                        waffle::Type::F32 => Operator::F32Const {
                            value: self.0.gen(),
                        },
                        waffle::Type::F64 => Operator::F64Const {
                            value: self.0.gen(),
                        },
                        waffle::Type::V128 => todo!(),
                        waffle::Type::FuncRef => todo!(),
...
impl FakeValBehavior for JustNormalCFF {
    fn fake(&mut self, f: &mut FunctionBody, b: Block, t: Type) -> Value {
        ...
        let v = f.add_value(ValueDef::Operator(
            match t {
                waffle::Type::I32 => Operator::I32Const { value: 0 },
                waffle::Type::I64 => Operator::I64Const { value: 0 },
                waffle::Type::F32 => Operator::F32Const { value: 0 },
                waffle::Type::F64 => Operator::F64Const { value: 0 },
                waffle::Type::V128 => todo!(),
                waffle::Type::FuncRef => todo!(),
            },
            ai.clone(),
...
pub fn cff(f: &mut FunctionBody, h: &mut (impl FakeValBehavior + CFFSpecial)) {
    ...
    let mut swc = vec![];
    let new = f.add_block();
    let fr = f.add_blockparam(new, waffle::Type::I32);
    let k = f
        .blocks
        .entries()
        .map(|a| a.0)
